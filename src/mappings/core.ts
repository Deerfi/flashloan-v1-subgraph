...
import { Pool as PoolContract, Mint, Burn, FlashLoan, Transfer, Sync } from '../types/templates/Pool/Pool'
...
export function handleSync(event: Sync): void {
  ...
  let flashLoan = FlashLoanFactory.load(FACTORY_ADDRESS)

  // reset factory liquidity by subtracting onluy tarcked liquidity
  flashLoan.totalLiquidityETH = flashLoan.totalLiquidityETH.minus(pool.trackedReserveETH as BigDecimal)
  ...
  if (bundle.ethPrice.notEqual(ZERO_BD)) {
    trackedLiquidityETH = getTrackedLiquidityUSD(pool.reserve, token as Token).div(
      bundle.ethPrice
    )
  } else {
  ...
  flashLoan.totalLiquidityETH = flashLoan.totalLiquidityETH.plus(trackedLiquidityETH)
  flashLoan.totalLiquidityUSD = flashLoan.totalLiquidityETH.times(bundle.ethPrice)
...
export function handleMint(event: Mint): void {
  ...
  flashLoan.txCount = flashLoan.txCount.plus(ONE_BI)
...
export function handleFlashLoan(event: FlashLoan): void {
  ...
  let trackedAmountUSD = getTrackedVolumeUSD(amountTotal, token as Token, pool as Pool)
